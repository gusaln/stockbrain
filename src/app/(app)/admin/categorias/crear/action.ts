...
export async function crearCategoria(prevState: any, formData: FormData) {
    ...
    const productoId = await createCategoria(validatedFields.data.nombre, validatedFields.data.descripcion);

    redirect("/admin/categorias?" + new URLSearchParams({ "message[success]": "Categoría registrada con éxito" }));
