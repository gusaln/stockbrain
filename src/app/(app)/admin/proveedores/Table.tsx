...
export function Table() {
    ...
    const { data, error, isFetching, isError } = useQuery({
        queryKey: ["/admin/proveedores/api", { page: page, limit: limit }],
        queryFn: async ({ queryKey }) => {
            ...
            return (await res.json()) as PaginatedResponse<Proveedor>;
        },
        initialData: {
            data: [],
            page: 1,
            limit: 10,
            total: 0,
        } as PaginatedResponse<Proveedor>,
    });
                ...
                {data.data?.map((p) => {
                    return (
                        <tr key={p.id}>
                            ...
                            <td>{p.direccion}</td>
                            <th>
                            <a href={`/admin/proveedores/${p.id}/editar`} className="btn btn-ghost btn-sm">
                                    editar
                                </a>
                                <a href={`/admin/proveedores/${p.id}/borrar`} className="btn btn-ghost btn-sm">
                                    borrar
                                </a>
                            </th>
                        </tr>
