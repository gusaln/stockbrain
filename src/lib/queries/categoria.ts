...
export async function createCategoria(nombre: string, descripcion: string) {
    const [result] = await runQuery(async function (connection) {
        return await connection.query("INSERT INTO categorias (nombre, descripcion) VALUES (?, ?)", [
            nombre,
            descripcion,
        ]);
    });
...
export async function existsCategoria(categoriaId: number) {
    const [result] = await runQuery(async function (connection) {
        return await connection.query("SELECT COUNT(id) as total FROM categorias WHERE id = ?", [categoriaId]);
    });
...
export async function getCategorias(search = undefined, pagination: Pagination = {}) {
    ...
    const [total, data] = await runQuery(async function (connection) {
        ...
        const [dataRes, dataField] = await connection.query("SELECT * FROM categorias LIMIT ?, ?", [offset, limit]);
...
export async function findCategoria(id: number) {
    const [data, dataField] = await runQuery(async function (connection) {
        const [dataRes, dataField] = await connection.query("SELECT * FROM categorias WHERE id = ?", [id]);
